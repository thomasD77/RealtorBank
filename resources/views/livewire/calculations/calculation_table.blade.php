                    ...
                    <table class="table table-striped table-bordered">
                        ...
                        @foreach($groupedSubCalculations as $categoryData)
                            <tr class="bg-light font-weight-bold" wire:key="category-{{ Str::slug($categoryData['category']) }}">
                                <td>
                                    <!-- Edit button voor Vetustate -->
                                    <button wire:click="editVetustate('{{ $categoryData['category'] }}')" class="btn btn-sm btn-warning">
                                        <i class="fa fa-edit"></i> {{ __('Bewerk Vetustate') }}
                                    </button>
                                </td>
                                <td colspan="5" class="text-center py-3">{{ $categoryData['category'] }}</td>
                            </tr>
                            ...
                            <tr class="font-weight-bold bg-light">
                                <td colspan="5" class="text-right">{{ __('Vetustate') }} ({{ $categoryData['vetustatePercentage'] }}%):</td>
                                <td class="text-right">-{{ number_format($categoryData['vetustateAmount'], 2, ',', '.') }} €</td>
                            </tr>
                            <tr class="font-weight-bold bg-light">
                                <td colspan="5" class="text-right">{{ __('Totaal na Vetustate') }}:</td>
                                <td class="text-right">{{ number_format($categoryData['finalTotal'], 2, ',', '.') }} €</td>
                            </tr>
                        @endforeach

                        <!-- Overall total sum row -->
                        <tr class="font-weight-bold bg-dark text-white">
                            <td colspan="5" class="text-right">{{ __('Eindtotaal') }}:</td>
                            <td class="text-right">{{ number_format($overallNetTotal, 2, ',', '.') }} €</td>
                        </tr>
                        </tbody>
                    </table>
                @else
